...
def extractor(inputs):
    ...
    # x = Conv2D(8, (6,6), activation='relu')(inputs)
    # # x = BatchNormalization()(x)
    # # x = LeakyReLU(alpha=0.1)(x)
    # x = MaxPooling2D(pool_size=(2, 2))(x)

    # x = Conv2D(16, (8,8), activation='relu')(x)
    # # x = BatchNormalization()(x)
    # # x = LeakyReLU(alpha=0.1)(x)
    # x = MaxPooling2D(pool_size=(2,2))(x)
...
X = np.load('data_prepared/train__X_rgb__merge_30game.npy')
Y = np.load('data_prepared/train__Y__merge_30game.npy')
...
# classification_output = Dense(units=1, activation='sigmoid', name='classification')(x)
x = Dense(2)(x)
classification_output = Activation('softmax')(x)
...
history= model.fit_generator(generator(X_train, Y_train, batch_size), epochs=epochs, steps_per_epoch=X_train.shape[0]//batch_size, 
                             validation_data=generator(X_val, Y_val, batch_size), validation_steps=X_val.shape[0]//batch_size, callbacks=[checkpoint, early_stopping])
