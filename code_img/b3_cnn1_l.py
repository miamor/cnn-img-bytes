...
def malware_input(if_random=True, if_training=True):
    ...
    print('filename_queue (v1)', filename_queue)
    ...
    # print('filename_queue (v2)', filename_queue)
    # iterator = iter(filename_queue)
...
def malware_train():
    ...
    label_batch_one_hot = tf.one_hot(
        tf.add(label_batch_out, label_offset), depth=5, on_value=1.0, off_value=0.0)
...
def malware_eval():
    ...
    logits_out = tf.reshape(malware_inference(
        image_batch_placeholder), [BATCH_SIZE, 5])
    ...
    with tf.Session() as sess:
        ...
        print('label_batch_out', label_batch_out)
        tpr_tot = 0.0
        far_tot = 0.0
