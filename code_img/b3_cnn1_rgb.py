...
DATA_DIR = './data_prepared/RGB_'
TRAINING_SET_SIZE = 761  # 5812
BATCH_SIZE = 5
IMAGE_SIZE = 224 # 64
...
def read_and_decode(filename_queue):
    ...
    image_object.image = tf.image.resize_image_with_crop_or_pad(
...
def malware_input(if_random=True, if_training=True):
    if (if_training):
        filenames = [os.path.join(
            DATA_DIR, 'train-0000%d-of-00002.tfrecord' % i) for i in range(0, 1)]
    else:
        filenames = [os.path.join(DATA_DIR, 'validation-0000%d-of-00002.tfrecord' % i)
                     for i in range(0, 1)]  # changing eval to train here!!
        # filenames = [os.path.join(DATA_DIR, 'train-0000%d-of-00002.tfrecord' % i) for i in range(0, 1)] ## changing eval to train here!!
    ...
    if if_random:
        ...
        min_queue_examples = int(
            TRAINING_SET_SIZE * min_fraction_of_examples_in_queue)
...
def malware_inference(image_batch):
    ...
    W_fc1 = weight_variable([7*7*256, 2048]) # for input size 224
    # W_fc1 = weight_variable([2*2*256, 2048]) # for input size 64
    b_fc1 = bias_variable([2048])

    h_pool5_flat = tf.reshape(h_pool5, [-1, 7*7*256]) # for input size 224
    # h_pool5_flat = tf.reshape(h_pool5, [-1, 2*2*256]) # for input size 64
...
def malware_train():
    image_batch_out, label_batch_out, filename_batch = malware_input(
        if_random=False, if_training=True)

    image_batch_placeholder = tf.placeholder(
        tf.float32, shape=[BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, 3])
    image_batch = tf.reshape(
        image_batch_out, (BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, 3))

    label_batch_placeholder = tf.placeholder(tf.float32, shape=[BATCH_SIZE, 2])
    label_offset = -tf.ones([BATCH_SIZE], dtype=tf.int64,
                            name='label_batch_offset')
    label_batch_one_hot = tf.one_hot(
        tf.add(label_batch_out, label_offset), depth=2, on_value=1.0, off_value=0.0)
    print('label_batch_one_hot', label_batch_one_hot)

    logits_out = malware_inference(image_batch_placeholder)
    print('logits_out', logits_out)
    loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(
        labels=label_batch_one_hot, logits=logits_out))
    # loss = tf.losses.mean_squared_error(labels=label_batch_placeholder, predictions=logits_out)

    # 0.007 is the learning rate.
    ...
    with tf.Session() as sess:
        ...
        # A coordinator for threads implements a mechanism to coordinate the termination of a set of threads.
        ...
        for i in range(TRAINING_SET_SIZE):  # * 100):
            ...
            _, infer_out, loss_out = sess.run([train_step, logits_out, loss], feed_dict={
                                              image_batch_placeholder: image_out, label_batch_placeholder: label_batch_one_hot_out})

            print('\n** i', i)
            # print('\t filename_out', filename_out)
            # print('\t image_out.shape', image_out.shape)
            print('\t label_out: ', label_out)
            print('\t label_batch_one_hot_out', label_batch_one_hot_out)
            print('\t infer_out: ', infer_out)
            print('\t loss, loss_out ', loss, loss_out)

            if (i % 100 == 0):  # change to 50
                saver.save(sess, './output/checkpoint-train.ckpt')
...
def malware_eval():
    image_batch_out, label_batch_out, filename_batch = malware_input(
        if_random=False, if_training=False)
    ...
    # tf.ones create a tensor mentioned shape(1st arg) and type(2nd arg) of 1's.
    label_offset = -tf.ones([BATCH_SIZE], dtype=tf.int64,
                            name='label_batch_offset')
    label_batch = tf.add(label_batch_out, label_offset)

    logits_out = tf.reshape(malware_inference(
        image_batch_placeholder), [BATCH_SIZE, 2])
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        saver.restore(sess, './output/checkpoint-train.ckpt')
        # A coordinator for threads implements a mechanism to coordinate the termination of a set of threads.
        coord = tf.train.Coordinator()
        threads = tf.train.start_queue_runners(coord=coord, sess=sess)
        ...
        for i in range(30):
            image_out, label_out, filename_out = sess.run(
                [image_batch, label_batch, filename_batch])

            accuracy_out, logits_batch_out = sess.run([accuracy, logits_batch], feed_dict={
                                                      image_batch_placeholder: image_out, label_tensor_placeholder: label_out})
            ...
            tot_bgn = np.sum(label_out == 0)
            tot_mal = np.sum(label_out == 1)

            tpr = 0.0
            far = 0.0
            for k, v in enumerate(label_out):
                # print(k, y[k], v)
                if v == 1 and logits_batch_out[k] == 1:
                    tpr += 1
                if v == 0 and logits_batch_out[k] == 1:
                    far += 1
            print('\t\t tpr/tot_mal = {}/{} = {}'.format(tpr, tot_mal, tpr/tot_mal))
            print('\t\t far/tot_bgn = {}/{} = {}'.format(far, tot_bgn, far/tot_bgn))
            tpr_tot += tpr/tot_mal
            far_tot += far/tot_bgn

        print('Accuracy: ', accuracy_accu/30)
        print('FAR: ', far_tot/30)
        print('TPR: ', tpr_tot/30)
